...
# ============================================================================
# Both may be given on the command line -- `bash eval.sh <task_name> [gpu_id]`, the form the
# README documents -- and fall back to these defaults when omitted.
TASK_NAME="${1:-click_alarmclock}"  # Task to test
GPU_ID="${2:-0}"                    # GPU to use
# Anything after those two is forwarded verbatim to eval_policy.py's --overrides, so a
# one-off run can set any deploy_policy.yml key, e.g. `bash eval.sh lift_pot 0 --test_num 5`.
EXTRA_OVERRIDES=("${@:3}")
