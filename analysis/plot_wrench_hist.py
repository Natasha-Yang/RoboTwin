...
end-effector link (aloha: `fl_link7`, `fl_link8`, `fr_link7`, `fr_link8`) and one per arm
(`left`, `right`), each arm being the sum of its own links. Plotting both at once would
draw that sum twice, so `--family` picks one; links win by default, since the arm total is
...
Reads the dataset's parquet shards directly (only the wrench columns), so it does not
...
# lands on one gripper and the warm half on the other, as in the debug plots.
THEME = {
...
def load_wrench(pattern, family="links"):
    ...
    The columns are discovered from the schema rather than named here, so the same script reads
    a dataset with both families, one with only the per-link columns, and an older per-arm one;
    ...
    prefix = "observation.wrench."
    cols = sorted(c for c in pq.read_schema(files[0]).names if c.startswith(prefix))
    if not cols:
        raise SystemExit(f"no {prefix}* columns in {files[0]} -- was data_type.wrench off?")
    arms = [c for c in cols if c[len(prefix):] in ARM_LABELS]
    ...
    series = {c[len(prefix):]: np.concatenate(chunks[c]).reshape(-1, len(COMPONENTS))
              for c in cols}
    rows = sum(len(c) for c in chunks[cols[0]])
    # Every column is padded at the same steps, so this is one mask over all of them.
