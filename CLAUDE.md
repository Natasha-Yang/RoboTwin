...
1. **Paths** — the repo hardcodes `/project/6101811/natashay/RoboTwin` (repo root) and
   ...
   Grep `6028519` too — it is the old Rorqual allocation, and stale fallbacks
   naming it have surfaced more than once when merging cluster branches.
2. **SLURM account + partition** — `--account=aip-florian7` (the only account this
   user is associated with here) and
