...
# Offline compute node: resolve the LeRobot dataset locally and block hub calls. The
# training config's `repo_id` is looked up directly underneath this, as
# $HF_LEROBOT_HOME/<repo_id> (CLAUDE.md §4.1), so it must be the dir holding the
# `NatashaYang/` level -- note the datasets here sit in per-modality sub-roots
# (`lerobot/data` for rgb, `lerobot/depth_data` for depth), so override accordingly.
export HF_LEROBOT_HOME="${HF_LEROBOT_HOME:-/home/natashay/project/def-florian7/lerobot}"
