...
def _ds_dist():
    """Import deepspeed's comm layer lazily.

    deepspeed is a *training-only* dependency here -- the only users are get_rank() and
    zero_first() below, both distributed-training helpers that inference never calls.
    Importing it at module scope runs its op-builder probe, which raises
    MissingCUDAException on any GPU node that has no CUDA toolkit on CUDA_HOME (torch
    ships its own runtime, so RoboTwin jobs do not load the cuda module).  That killed
    `script/eval_policy.py` at import, since models/motus.py imports this module.
    """
